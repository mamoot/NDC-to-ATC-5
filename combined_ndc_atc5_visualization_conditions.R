...
do_snomedct <- TRUE # If true, will request SNOMED CT from RxNorm.
do_meshpa <- FALSE # If true, will request MESH Pharmacological Actions from RxNorm.
do_ingredients <- TRUE # If true, will request the drug's ingredients from RxNorm.
...
org<- gganatogram(data=organPlot, fillOutline='#a6bddb', organism='human', sex='male', fill="value")+
...
top_ten<-data.frame(group=c("Lipid Modifying Agents","Diuretics","Calcium Channel Blockers","Renin-Angiotensin system agents",
                            "Beta blocking agents","Acid-relateted disorders","Diabetic drugs","Analgesics","Corticosteroids","Anxiolytics"),
                    value=c(15.3,14.2,12.4,11.3,11.1,8.7,8.4,7.4,5.7,5.5))
top_ten <- top_ten %>%
  arrange(desc(group)) %>%
  mutate(lab.ypos = cumsum(value) - 0.5*value)
top_ten

bp<- ggplot(df, aes(x="", y=value, fill=group))+ggtitle("Top 10 ATC Level 1 (%)")+
  geom_bar(width = 1, stat = "identity")+theme(axis.title=element_blank())

png(file="Top_ten_ATC_Level_1_bar_plot.png", width=400, height=600)
bp
dev.off()


p <- ggplot(df, aes(x=2, y=value, fill=group)) +
...
p <- p +
  ...
  geom_text(aes(y = lab.ypos, label = value), color = "black")+xlim(1, 2.5)

png(file="Top_ten_ATC_Level_1_pie_plot.png", width=600, height=600)
p
dev.off()
...
png(file="Top_ten_drug_classes_bar_plot.png", width=400, height=600)
bp
dev.off()
#pie <- bp + coord_polar("y", start=0)+theme_void()
#pie

p <- ggplot(top_ten, aes(x=2, y=value, fill=group)) +
...
png(file="Top_ten_ATC_classes_pie_plot.png", width=600, height=600)
p
dev.off()
...
write.csv(dat, 'NDC_to_ATC_with_inferred_conditions.csv', row.names = F)
